...
projection's explicit user filter. Source manifest v25 adds `training/dataset_catalog.py`, the
...
manifest v25 includes the archive, export and artifact modules for new fingerprints only.
...
fingerprints use source manifest v25, including the three relocated RunPod source files;
...
falls back to the clock, while string "0" resolves to zero. Manifest v25 includes the relocated
...
v25 adds `training/runner.py`, `training/submission.py` and `training/local_process.py` for
...
copy only the top level. The current source manifest is v25 and adds `training/user_state.py`,
...
failures are not retried. Source manifest v25 includes both moved executor implementations; historical
...
and parse only the first epochs/imgsz command argument. Source manifest v25 covers the moved runtime, lifecycle and view implementations;
...
extraction. Source manifest v25 includes the three migrated pipeline implementations;
...
v25 includes the moved training identity/storage implementations. The shared
...
business API setting. Source manifest v25 includes `training/task_lookup.py`,
...
v25 includes the three migrated warmup sources; historical snapshots remain unchanged.
...
source manifest v25 includes `detection/model_selection.py` and `detection/local_models.py`
...
rules. Manifest v25 now includes the actual task projection and catalog files that
...
Source manifest v25 adds the three task modules that normalize counts and select
...
callbacks preserve argument evaluation and TypeError. Source manifest v25 covers
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v25 lists the
...
Source manifest v25 includes all five migrated detection files, preserving source
...
v25 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v25 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v25; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v25
...
manifest v25 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v25; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v25 with the migrated media
...
at execution time. The prompt source manifest uses v25, but edits to listed
...
fingerprints use manifest v25, which includes the actual `accessories/preparation.py`
