...
retry or model policy is added. Source manifest v41 includes the actual relocated artifact source
...
is added. Source manifest v41 includes both relocated files only for new source fingerprints.
...
No environment switch, retry, model or identity scope is added. Source manifest v41 records the
...
is added. Source manifest v41 includes both relocated source files for new task fingerprints.
...
checks and all request/response defaults are retained. New source fingerprints use manifest v41.
...
and all background settings remain unchanged. New task source fingerprints use manifest v41.
...
training configuration before its existing 409. New source manifest v41 records all four relocated
...
Source manifest v41 includes the relocated query, submission and plan-storage sources for new
...
the original short-circuit reads. Source manifest v41 includes the relocated renderer; stored task
...
thresholds and OpenCV transforms remain. New task source manifest v41 includes the three actual
...
unchanged. Source manifest v41 includes both actual relocated background input files for new task
...
projection's explicit user filter. Source manifest v41 adds `training/dataset_catalog.py`, the
...
manifest v41 includes the archive, export and artifact modules for new fingerprints only.
...
fingerprints use source manifest v41, including the three relocated RunPod source files;
...
falls back to the clock, while string "0" resolves to zero. Manifest v41 includes the relocated
...
v41 adds `training/runner.py`, `training/submission.py` and `training/local_process.py` for
...
copy only the top level. The current source manifest is v41 and adds `training/user_state.py`,
...
failures are not retried. Source manifest v41 includes both moved executor implementations; historical
...
and parse only the first epochs/imgsz command argument. Source manifest v41 covers the moved runtime, lifecycle and view implementations;
...
extraction. Source manifest v41 includes the three migrated pipeline implementations;
...
v41 includes the moved training identity/storage implementations. The shared
...
business API setting. Source manifest v41 includes `training/task_lookup.py`,
...
v41 includes the three migrated warmup sources; historical snapshots remain unchanged.
...
source manifest v41 includes `detection/model_selection.py` and `detection/local_models.py`
...
rules. Manifest v41 now includes the actual task projection and catalog files that
...
Source manifest v41 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v41 lists the
...
Source manifest v41 includes all five migrated detection files, preserving source
...
v41 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v41 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v41; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v41
...
manifest v41 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v41; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v41 with the migrated media
...
at execution time. The prompt source manifest uses v41, but edits to listed
...
fingerprints use manifest v41, which includes the actual `accessories/preparation.py`
