...
unchanged. Source manifest v26 includes both actual relocated background input files for new task
...
projection's explicit user filter. Source manifest v26 adds `training/dataset_catalog.py`, the
...
manifest v26 includes the archive, export and artifact modules for new fingerprints only.
...
fingerprints use source manifest v26, including the three relocated RunPod source files;
...
falls back to the clock, while string "0" resolves to zero. Manifest v26 includes the relocated
...
v26 adds `training/runner.py`, `training/submission.py` and `training/local_process.py` for
...
copy only the top level. The current source manifest is v26 and adds `training/user_state.py`,
...
failures are not retried. Source manifest v26 includes both moved executor implementations; historical
...
and parse only the first epochs/imgsz command argument. Source manifest v26 covers the moved runtime, lifecycle and view implementations;
...
extraction. Source manifest v26 includes the three migrated pipeline implementations;
...
v26 includes the moved training identity/storage implementations. The shared
...
business API setting. Source manifest v26 includes `training/task_lookup.py`,
...
v26 includes the three migrated warmup sources; historical snapshots remain unchanged.
...
source manifest v26 includes `detection/model_selection.py` and `detection/local_models.py`
...
rules. Manifest v26 now includes the actual task projection and catalog files that
...
Source manifest v26 adds the three task modules that normalize counts and select
...
callbacks preserve argument evaluation and TypeError. Source manifest v26 covers
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v26 lists the
...
Source manifest v26 includes all five migrated detection files, preserving source
...
v26 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v26 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v26; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v26
...
manifest v26 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v26; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v26 with the migrated media
...
at execution time. The prompt source manifest uses v26, but edits to listed
...
fingerprints use manifest v26, which includes the actual `accessories/preparation.py`
