...
Source manifest v11 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v11 lists the
...
Source manifest v11 includes all five migrated detection files, preserving source
...
v11 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v11 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v11; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v11
...
manifest v11 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v11; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v11 with the migrated media
...
at execution time. The prompt source manifest uses v11, but edits to listed
...
fingerprints use manifest v11, which includes the actual `accessories/preparation.py`
