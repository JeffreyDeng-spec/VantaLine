...
rules. Manifest v13 now includes the actual task projection and catalog files that
...
Source manifest v13 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v13 lists the
...
Source manifest v13 includes all five migrated detection files, preserving source
...
v13 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v13 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v13; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v13
...
manifest v13 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v13; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v13 with the migrated media
...
at execution time. The prompt source manifest uses v13, but edits to listed
...
fingerprints use manifest v13, which includes the actual `accessories/preparation.py`
