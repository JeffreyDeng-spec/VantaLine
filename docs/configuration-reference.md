...
rules. Manifest v12 now includes the actual task projection and catalog files that
...
Source manifest v12 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v12 lists the
...
Source manifest v12 includes all five migrated detection files, preserving source
...
v12 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v12 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v12; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v12
...
manifest v12 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v12; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v12 with the migrated media
...
at execution time. The prompt source manifest uses v12, but edits to listed
...
fingerprints use manifest v12, which includes the actual `accessories/preparation.py`
