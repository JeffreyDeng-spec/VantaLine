...
and all background settings remain unchanged. New task source fingerprints use manifest v36.
...
training configuration before its existing 409. New source manifest v36 records all four relocated
...
Source manifest v36 includes the relocated query, submission and plan-storage sources for new
...
the original short-circuit reads. Source manifest v36 includes the relocated renderer; stored task
...
thresholds and OpenCV transforms remain. New task source manifest v36 includes the three actual
...
unchanged. Source manifest v36 includes both actual relocated background input files for new task
...
projection's explicit user filter. Source manifest v36 adds `training/dataset_catalog.py`, the
...
manifest v36 includes the archive, export and artifact modules for new fingerprints only.
...
fingerprints use source manifest v36, including the three relocated RunPod source files;
...
falls back to the clock, while string "0" resolves to zero. Manifest v36 includes the relocated
...
v36 adds `training/runner.py`, `training/submission.py` and `training/local_process.py` for
...
copy only the top level. The current source manifest is v36 and adds `training/user_state.py`,
...
failures are not retried. Source manifest v36 includes both moved executor implementations; historical
...
and parse only the first epochs/imgsz command argument. Source manifest v36 covers the moved runtime, lifecycle and view implementations;
...
extraction. Source manifest v36 includes the three migrated pipeline implementations;
...
v36 includes the moved training identity/storage implementations. The shared
...
business API setting. Source manifest v36 includes `training/task_lookup.py`,
...
v36 includes the three migrated warmup sources; historical snapshots remain unchanged.
...
source manifest v36 includes `detection/model_selection.py` and `detection/local_models.py`
...
rules. Manifest v36 now includes the actual task projection and catalog files that
...
Source manifest v36 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v36 lists the
...
Source manifest v36 includes all five migrated detection files, preserving source
...
v36 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v36 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v36; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v36
...
manifest v36 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v36; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v36 with the migrated media
...
at execution time. The prompt source manifest uses v36, but edits to listed
...
fingerprints use manifest v36, which includes the actual `accessories/preparation.py`
