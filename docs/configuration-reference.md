...
v16 includes the moved training identity/storage implementations. The shared
...
business API setting. Source manifest v16 includes `training/task_lookup.py`,
...
v16 includes the three migrated warmup sources; historical snapshots remain unchanged.
...
source manifest v16 includes `detection/model_selection.py` and `detection/local_models.py`
...
rules. Manifest v16 now includes the actual task projection and catalog files that
...
Source manifest v16 adds the three task modules that normalize counts and select
...
the incoming engine continues to use PP-OCRv6-medium. Source manifest v16 lists the
...
Source manifest v16 includes all five migrated detection files, preserving source
...
v16 includes `text_inspection/incoming_execution.py`, the actual CLAHE/OCR input
...
request identity remain lazy providers. Manifest v16 includes the actual migrated input
...
unchanged. Prompt-source manifest remains v16; no model-input source moved here.
...
admission and the business prompt version unchanged. Prompt-source manifest v16
...
manifest v16 continues; stored task bindings and historical fingerprints are retained.
...
Source manifest is v16; these helpers do not move model-input producers.
...
messages remain. New source fingerprints use manifest v16 with the migrated media
...
at execution time. The prompt source manifest uses v16, but edits to listed
...
fingerprints use manifest v16, which includes the actual `accessories/preparation.py`
