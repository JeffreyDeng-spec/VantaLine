...
original step order. Deploy/restore all matching modules and source manifest v21 together.
...
Ship/restore the matching v21 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v21.
...
source manifest v21. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v21 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v21.
...
source manifest v21 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v21 as one complete release.
...
with matching manifest v21, retaining historical snapshots. This batch does not
