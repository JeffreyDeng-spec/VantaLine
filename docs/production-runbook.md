...
modules with matching source manifest v67 as one complete release.
...
Deploy/rollback the complete workflow package with matching source manifest v67.
...
Deploy or restore the renderer and its ports with matching source manifest v67 together.
...
original pass flag. Deploy or roll back all three layout modules and source manifest v67 together.
...
with matching source manifest v67 as a complete release.
...
source manifest v67; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v67.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v67.
...
v67 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v67 together.
...
Ship/restore the matching v67 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v67.
...
source manifest v67. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v67 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v67.
...
source manifest v67 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v67 as one complete release.
...
with matching manifest v67, retaining historical snapshots. This batch does not
