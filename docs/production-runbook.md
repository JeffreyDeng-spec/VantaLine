...
runtime records and deploy both new modules with source manifest v16.
...
source manifest v16. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v16 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v16.
...
source manifest v16 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v16 as one complete release.
...
with matching manifest v16, retaining historical snapshots. This batch does not
