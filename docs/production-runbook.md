...
source manifest v25; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v25.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v25.
...
v25 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v25 together.
...
Ship/restore the matching v25 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v25.
...
source manifest v25. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v25 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v25.
...
source manifest v25 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v25 as one complete release.
...
with matching manifest v25, retaining historical snapshots. This batch does not
