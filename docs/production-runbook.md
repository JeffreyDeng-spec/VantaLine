...
label-detection timeout/late-response rules. Restore the complete package with manifest v23.
...
v23 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v23 together.
...
Ship/restore the matching v23 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v23.
...
source manifest v23. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v23 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v23.
...
source manifest v23 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v23 as one complete release.
...
with matching manifest v23, retaining historical snapshots. This batch does not
