...
restore the complete package with matching source manifest v24.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v24.
...
v24 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v24 together.
...
Ship/restore the matching v24 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v24.
...
source manifest v24. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v24 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v24.
...
source manifest v24 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v24 as one complete release.
...
with matching manifest v24, retaining historical snapshots. This batch does not
