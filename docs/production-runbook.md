...
runtime records and deploy both new modules with source manifest v17.
...
source manifest v17. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v17 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v17.
...
source manifest v17 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v17 as one complete release.
...
with matching manifest v17, retaining historical snapshots. This batch does not
