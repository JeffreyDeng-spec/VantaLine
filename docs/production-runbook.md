...
with matching source manifest v26 as a complete release.
...
source manifest v26; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v26.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v26.
...
v26 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v26 together.
...
Ship/restore the matching v26 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v26.
...
source manifest v26. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v26 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v26.
...
source manifest v26 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v26 as one complete release.
...
with matching manifest v26, retaining historical snapshots. This batch does not
