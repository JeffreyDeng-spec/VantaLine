...
modules with matching source manifest v32 as one complete release.
...
Deploy/rollback the complete workflow package with matching source manifest v32.
...
Deploy or restore the renderer and its ports with matching source manifest v32 together.
...
original pass flag. Deploy or roll back all three layout modules and source manifest v32 together.
...
with matching source manifest v32 as a complete release.
...
source manifest v32; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v32.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v32.
...
v32 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v32 together.
...
Ship/restore the matching v32 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v32.
...
source manifest v32. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v32 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v32.
...
source manifest v32 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v32 as one complete release.
...
with matching manifest v32, retaining historical snapshots. This batch does not
