...
Ship/restore the matching v20 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v20.
...
source manifest v20. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v20 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v20.
...
source manifest v20 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v20 as one complete release.
...
with matching manifest v20, retaining historical snapshots. This batch does not
