...
modules with matching source manifest v64 as one complete release.
...
Deploy/rollback the complete workflow package with matching source manifest v64.
...
Deploy or restore the renderer and its ports with matching source manifest v64 together.
...
original pass flag. Deploy or roll back all three layout modules and source manifest v64 together.
...
with matching source manifest v64 as a complete release.
...
source manifest v64; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v64.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v64.
...
v64 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v64 together.
...
Ship/restore the matching v64 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v64.
...
source manifest v64. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v64 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v64.
...
source manifest v64 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v64 as one complete release.
...
with matching manifest v64, retaining historical snapshots. This batch does not
