...
modules with matching source manifest v78 as one complete release.
...
Deploy/rollback the complete workflow package with matching source manifest v78.
...
Deploy or restore the renderer and its ports with matching source manifest v78 together.
...
original pass flag. Deploy or roll back all three layout modules and source manifest v78 together.
...
with matching source manifest v78 as a complete release.
...
source manifest v78; do not copy individual catalog files into production.
...
restore the complete package with matching source manifest v78.
...
label-detection timeout/late-response rules. Restore the complete package with manifest v78.
...
v78 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v78 together.
...
Ship/restore the matching v78 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v78.
...
source manifest v78. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v78 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v78.
...
source manifest v78 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v78 as one complete release.
...
with matching manifest v78, retaining historical snapshots. This batch does not
