...
v22 manifest; do not repair a rollout by copying individual generation files.
...
original step order. Deploy/restore all matching modules and source manifest v22 together.
...
Ship/restore the matching v22 manifest and modules together; historical snapshots are retained.
...
runtime records and deploy both new modules with source manifest v22.
...
source manifest v22. Existing records and historical model snapshots are preserved.
...
no thread coordination or lifecycle idempotence; deploy/restore manifest v22 and all
...
warmup threads or restart behavior. Roll back the complete package with manifest v22.
...
source manifest v22 with the entire immutable release.
...
parameters. Deploy/rollback all OCR modules and manifest v22 as one complete release.
...
with matching manifest v22, retaining historical snapshots. This batch does not
