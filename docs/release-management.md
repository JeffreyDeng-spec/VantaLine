...
source manifest advances to v19 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v19 in the whole release.
...
commands and topology remain unchanged. Source manifest v19 now includes both
...
new modules and source manifest v19 with the complete immutable release. This batch
...
warmup modules and manifest v19 together. Worker topology and release commands remain.
...
new modules with manifest v19. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v19 records the new model-input
...
five OCR modules with source manifest v19. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v19
...
Source manifest v19 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v19 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v19 together;
...
Manifest v19 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v19, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v19 includes
