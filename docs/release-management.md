...
Source manifest v34 includes their actual sources for new task fingerprints and leaves historical
...
Source manifest v34 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v34
...
v34 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v34
...
with source manifest v34 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v34 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v34
...
modules and source manifest v34 in the same immutable release. No dependency, migration, route,
...
with source manifest v34 in the complete immutable release. Routes, storage schema and process
...
source manifest v34 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v34. This batch changes no request/worker contract, dependency, service,
...
manifest v34 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v34. Training still runs through the existing
...
with source manifest v34 in the same immutable package. No migration, environment setting,
...
source manifest advances to v34 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v34 in the whole release.
...
commands and topology remain unchanged. Source manifest v34 now includes both
...
new modules and source manifest v34 with the complete immutable release. This batch
...
warmup modules and manifest v34 together. Worker topology and release commands remain.
...
new modules with manifest v34. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v34 records the new model-input
...
five OCR modules with source manifest v34. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v34
...
Source manifest v34 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v34 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v34 together;
...
Manifest v34 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v34, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v34 includes
