...
source manifest advances to v18 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v18 in the whole release.
...
commands and topology remain unchanged. Source manifest v18 now includes both
...
new modules and source manifest v18 with the complete immutable release. This batch
...
warmup modules and manifest v18 together. Worker topology and release commands remain.
...
new modules with manifest v18. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v18 records the new model-input
...
five OCR modules with source manifest v18. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v18
...
Source manifest v18 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v18 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v18 together;
...
Manifest v18 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v18, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v18 includes
