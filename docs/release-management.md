...
Ship renderer, its typed ports and source manifest v28 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v28
...
modules and source manifest v28 in the same immutable release. No dependency, migration, route,
...
with source manifest v28 in the complete immutable release. Routes, storage schema and process
...
source manifest v28 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v28. This batch changes no request/worker contract, dependency, service,
...
manifest v28 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v28. Training still runs through the existing
...
with source manifest v28 in the same immutable package. No migration, environment setting,
...
source manifest advances to v28 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v28 in the whole release.
...
commands and topology remain unchanged. Source manifest v28 now includes both
...
new modules and source manifest v28 with the complete immutable release. This batch
...
warmup modules and manifest v28 together. Worker topology and release commands remain.
...
new modules with manifest v28. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v28 records the new model-input
...
five OCR modules with source manifest v28. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v28
...
Source manifest v28 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v28 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v28 together;
...
Manifest v28 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v28, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v28 includes
