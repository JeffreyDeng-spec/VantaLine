...
resource and detection endpoint checks. Manifest v12 records the new model-input
...
five OCR modules with source manifest v12. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v12
...
Source manifest v12 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v12 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v12 together;
...
Manifest v12 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v12, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v12 includes
