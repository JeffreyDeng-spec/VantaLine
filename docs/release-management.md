...
source manifest v24 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v24. This batch changes no request/worker contract, dependency, service,
...
manifest v24 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v24. Training still runs through the existing
...
with source manifest v24 in the same immutable package. No migration, environment setting,
...
source manifest advances to v24 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v24 in the whole release.
...
commands and topology remain unchanged. Source manifest v24 now includes both
...
new modules and source manifest v24 with the complete immutable release. This batch
...
warmup modules and manifest v24 together. Worker topology and release commands remain.
...
new modules with manifest v24. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v24 records the new model-input
...
five OCR modules with source manifest v24. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v24
...
Source manifest v24 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v24 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v24 together;
...
Manifest v24 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v24, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v24 includes
