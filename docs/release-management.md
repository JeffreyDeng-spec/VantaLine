...
Remote training and worker compatibility modules ship with source manifest v38 as one immutable
...
v38. The three relocated business source files are included for new task fingerprints; stored model
snapshots remain intact. No route, migration, worker topology or release command changes.

Background Codex transport, task runner and submission ship together with source manifest v38.
...
The three background write modules ship together with source manifest v38, which records their
...
Source manifest v38 includes their actual sources for new task fingerprints and leaves historical
...
Source manifest v38 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v38
...
v38 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v38
...
with source manifest v38 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v38 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v38
...
modules and source manifest v38 in the same immutable release. No dependency, migration, route,
...
with source manifest v38 in the complete immutable release. Routes, storage schema and process
...
source manifest v38 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v38. This batch changes no request/worker contract, dependency, service,
...
manifest v38 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v38. Training still runs through the existing
...
with source manifest v38 in the same immutable package. No migration, environment setting,
...
source manifest advances to v38 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v38 in the whole release.
...
commands and topology remain unchanged. Source manifest v38 now includes both
...
new modules and source manifest v38 with the complete immutable release. This batch
...
warmup modules and manifest v38 together. Worker topology and release commands remain.
...
new modules with manifest v38. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v38 records the new model-input
...
five OCR modules with source manifest v38. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v38
...
Source manifest v38 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v38 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v38 together;
...
Manifest v38 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v38, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v38 includes
