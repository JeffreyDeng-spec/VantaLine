...
new modules and source manifest v15 with the complete immutable release. This batch
...
warmup modules and manifest v15 together. Worker topology and release commands remain.
...
new modules with manifest v15. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v15 records the new model-input
...
five OCR modules with source manifest v15. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v15
...
Source manifest v15 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v15 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v15 together;
...
Manifest v15 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v15, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v15 includes
