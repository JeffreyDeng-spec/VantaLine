...
Source manifest v33 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v33
...
v33 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v33
...
with source manifest v33 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v33 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v33
...
modules and source manifest v33 in the same immutable release. No dependency, migration, route,
...
with source manifest v33 in the complete immutable release. Routes, storage schema and process
...
source manifest v33 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v33. This batch changes no request/worker contract, dependency, service,
...
manifest v33 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v33. Training still runs through the existing
...
with source manifest v33 in the same immutable package. No migration, environment setting,
...
source manifest advances to v33 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v33 in the whole release.
...
commands and topology remain unchanged. Source manifest v33 now includes both
...
new modules and source manifest v33 with the complete immutable release. This batch
...
warmup modules and manifest v33 together. Worker topology and release commands remain.
...
new modules with manifest v33. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v33 records the new model-input
...
five OCR modules with source manifest v33. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v33
...
Source manifest v33 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v33 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v33 together;
...
Manifest v33 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v33, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v33 includes
