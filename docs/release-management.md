...
with source manifest v23. This batch changes no request/worker contract, dependency, service,
...
manifest v23 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v23. Training still runs through the existing
...
with source manifest v23 in the same immutable package. No migration, environment setting,
...
source manifest advances to v23 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v23 in the whole release.
...
commands and topology remain unchanged. Source manifest v23 now includes both
...
new modules and source manifest v23 with the complete immutable release. This batch
...
warmup modules and manifest v23 together. Worker topology and release commands remain.
...
new modules with manifest v23. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v23 records the new model-input
...
five OCR modules with source manifest v23. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v23
...
Source manifest v23 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v23 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v23 together;
...
Manifest v23 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v23, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v23 includes
