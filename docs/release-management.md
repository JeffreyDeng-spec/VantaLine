...
Presence inspection, its narrow ports and manifest v51 are one immutable release unit. CI runs
...
Detection orchestration, its ports and source manifest v51 ship in one immutable package.
...
The annotation module and source manifest v51 are packaged with the existing web service.
...
The presence result module and manifest v51 ship in the same immutable release. CI runs the
...
Failure projection/result modules ship with source manifest v51 as part of the immutable package.
...
Presence payload/validation modules and source manifest v51 are packaged together. CI adds their
...
Accessory lookup and detection requirements ship with source manifest v51 in the same immutable
...
Retired task/refresh modules and source manifest v51 ship together in the immutable release.
...
The retired request/status module ships with source manifest v51 in the complete immutable
...
The watcher module and source manifest v51 ship with the full immutable package. Startup hook
...
Worker artifact import and source manifest v51 ship with the complete immutable release. There
...
Worker bundle metadata/submission and source manifest v51 ship as one immutable package.
...
Streamed worker transport and progress modules ship with source manifest v51 as one immutable
...
Remote training and worker compatibility modules ship with source manifest v51 as one immutable
...
v51. The three relocated business source files are included for new task fingerprints; stored model
snapshots remain intact. No route, migration, worker topology or release command changes.

Background Codex transport, task runner and submission ship together with source manifest v51.
...
The three background write modules ship together with source manifest v51, which records their
...
Source manifest v51 includes their actual sources for new task fingerprints and leaves historical
...
Source manifest v51 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v51
...
v51 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v51
...
with source manifest v51 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v51 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v51
...
modules and source manifest v51 in the same immutable release. No dependency, migration, route,
...
with source manifest v51 in the complete immutable release. Routes, storage schema and process
...
source manifest v51 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v51. This batch changes no request/worker contract, dependency, service,
...
manifest v51 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v51. Training still runs through the existing
...
with source manifest v51 in the same immutable package. No migration, environment setting,
...
source manifest advances to v51 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v51 in the whole release.
...
commands and topology remain unchanged. Source manifest v51 now includes both
...
new modules and source manifest v51 with the complete immutable release. This batch
...
warmup modules and manifest v51 together. Worker topology and release commands remain.
...
new modules with manifest v51. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v51 records the new model-input
...
callbacks preserve argument evaluation and TypeError. Source manifest v51 covers
...
five OCR modules with source manifest v51. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v51
...
Source manifest v51 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v51 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v51 together;
...
Manifest v51 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v51, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v51 includes
