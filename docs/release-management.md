...
Background Codex transport, task runner and submission ship together with source manifest v36.
...
The three background write modules ship together with source manifest v36, which records their
...
Source manifest v36 includes their actual sources for new task fingerprints and leaves historical
...
Source manifest v36 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v36
...
v36 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v36
...
with source manifest v36 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v36 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v36
...
modules and source manifest v36 in the same immutable release. No dependency, migration, route,
...
with source manifest v36 in the complete immutable release. Routes, storage schema and process
...
source manifest v36 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v36. This batch changes no request/worker contract, dependency, service,
...
manifest v36 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v36. Training still runs through the existing
...
with source manifest v36 in the same immutable package. No migration, environment setting,
...
source manifest advances to v36 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v36 in the whole release.
...
commands and topology remain unchanged. Source manifest v36 now includes both
...
new modules and source manifest v36 with the complete immutable release. This batch
...
warmup modules and manifest v36 together. Worker topology and release commands remain.
...
new modules with manifest v36. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v36 records the new model-input
...
five OCR modules with source manifest v36. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v36
...
Source manifest v36 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v36 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v36 together;
...
Manifest v36 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v36, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v36 includes
