...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v27
...
modules and source manifest v27 in the same immutable release. No dependency, migration, route,
...
with source manifest v27 in the complete immutable release. Routes, storage schema and process
...
source manifest v27 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v27. This batch changes no request/worker contract, dependency, service,
...
manifest v27 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v27. Training still runs through the existing
...
with source manifest v27 in the same immutable package. No migration, environment setting,
...
source manifest advances to v27 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v27 in the whole release.
...
commands and topology remain unchanged. Source manifest v27 now includes both
...
new modules and source manifest v27 with the complete immutable release. This batch
...
warmup modules and manifest v27 together. Worker topology and release commands remain.
...
new modules with manifest v27. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v27 records the new model-input
...
five OCR modules with source manifest v27. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v27
...
Source manifest v27 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v27 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v27 together;
...
Manifest v27 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v27, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v27 includes
