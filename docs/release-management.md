...
training modules together with source manifest v21. Training still runs through the existing
...
with source manifest v21 in the same immutable package. No migration, environment setting,
...
source manifest advances to v21 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v21 in the whole release.
...
commands and topology remain unchanged. Source manifest v21 now includes both
...
new modules and source manifest v21 with the complete immutable release. This batch
...
warmup modules and manifest v21 together. Worker topology and release commands remain.
...
new modules with manifest v21. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v21 records the new model-input
...
five OCR modules with source manifest v21. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v21
...
Source manifest v21 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v21 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v21 together;
...
Manifest v21 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v21, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v21 includes
