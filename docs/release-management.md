...
manifest v22 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v22. Training still runs through the existing
...
with source manifest v22 in the same immutable package. No migration, environment setting,
...
source manifest advances to v22 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v22 in the whole release.
...
commands and topology remain unchanged. Source manifest v22 now includes both
...
new modules and source manifest v22 with the complete immutable release. This batch
...
warmup modules and manifest v22 together. Worker topology and release commands remain.
...
new modules with manifest v22. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v22 records the new model-input
...
five OCR modules with source manifest v22. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v22
...
Source manifest v22 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v22 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v22 together;
...
Manifest v22 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v22, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v22 includes
