...
Ship the two stores, pure state policy and source manifest v17 in the whole release.
...
commands and topology remain unchanged. Source manifest v17 now includes both
...
new modules and source manifest v17 with the complete immutable release. This batch
...
warmup modules and manifest v17 together. Worker topology and release commands remain.
...
new modules with manifest v17. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v17 records the new model-input
...
five OCR modules with source manifest v17. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v17
...
Source manifest v17 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v17 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v17 together;
...
Manifest v17 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v17, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v17 includes
