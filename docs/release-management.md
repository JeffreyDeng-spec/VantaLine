Label native-run batching ships as one compatible Web release. It changes only first-page read/query grouping and source manifest v131 (341 entries); the 15-minute cursor format, embedded worker, database schema and write lock remain unchanged. Accepted-source projection is replayed locally; CI runs candidate contract checks and isolated PostgreSQL benchmarks before merge. Roll back to the previous complete immutable release without rewriting task or call evidence.
