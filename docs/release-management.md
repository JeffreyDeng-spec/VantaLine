...
with source manifest v20 in the same immutable package. No migration, environment setting,
...
source manifest advances to v20 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v20 in the whole release.
...
commands and topology remain unchanged. Source manifest v20 now includes both
...
new modules and source manifest v20 with the complete immutable release. This batch
...
warmup modules and manifest v20 together. Worker topology and release commands remain.
...
new modules with manifest v20. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v20 records the new model-input
...
five OCR modules with source manifest v20. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v20
...
Source manifest v20 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v20 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v20 together;
...
Manifest v20 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v20, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v20 includes
