...
source manifest v78. Existing lifecycle, worker topology and installer are unchanged.
...
the same release as the root adapters. Source manifest v78 records their actual modules.
...
Manifest v78 includes their actual source files. Rollback restores the previous complete
...
root adapters in the same release. Manifest v78 includes the actual source modules. Rollback
...
root adapters in one release, covered by source manifest v78. Restore the previous complete
...
and root adapters. Source manifest v78 records the actual source files. Rollback restores
...
adapters in one release. Source manifest v78 records actual prompt sources; rollback restores
...
one release. Source manifest v78 includes all implementations. Restore the previous complete
...
adapters in one release. Source manifest v78 fingerprints the implementation. Restore the
...
one release, fingerprinted by source manifest v78. Rollback restores the complete previous
...
source manifest v78. The single task snapshot binding remains on the application entry. Existing
...
Agent settings HTTP handlers, application adapters and source manifest v78 ship together.
...
Agent protocol, invocation, discovery and recommendation services and source manifest v78
...
v78 ship together. Synthetic contracts cover permissions, callback ordering, failure evidence and
independent instances. Rollback restores the previous complete package and retains runtime state.

Key identity, local secret-file services, typed capabilities and source manifest v78 deploy
...
Provider key registry adapters, business module, capabilities and source manifest v78 ship
...
Provider configuration defaults, validation, public URL formatting and manifest v78 ship
...
Legacy JSON/image settings, root callbacks and manifest v78 deploy together. Required CI
...
Status projection services, root adapters and manifest v78 deploy as one immutable release.
...
Provider orchestration, root adapters, explicit capabilities and manifest v78 deploy together.
...
Agnes/Qwen image transports and manifest v78 deploy together with their root adapters.
...
v78 ship together. Offline checks preserve request, resource, dependency-capture and accounting
...
with source manifest v78. Offline contracts check original requests, usage, failure evidence,
...
Provider-foundation modules, their root aliases/adapters and source manifest v78 ship in one
...
source manifest v78 ship together. CI runs synthetic media contracts with existing detection and
model checks. No publisher or service topology changes are introduced in this extraction.

Image/video upload services, video projections and manifest v78 ship as one immutable package.
...
Profile-cache policy, store, flow and manifest v78 ship as a single immutable package. CI runs
...
Presence inspection, its narrow ports and manifest v78 are one immutable release unit. CI runs
...
Detection orchestration, its ports and source manifest v78 ship in one immutable package.
...
The annotation module and source manifest v78 are packaged with the existing web service.
...
The presence result module and manifest v78 ship in the same immutable release. CI runs the
...
Failure projection/result modules ship with source manifest v78 as part of the immutable package.
...
Presence payload/validation modules and source manifest v78 are packaged together. CI adds their
...
Accessory lookup and detection requirements ship with source manifest v78 in the same immutable
...
Retired task/refresh modules and source manifest v78 ship together in the immutable release.
...
The retired request/status module ships with source manifest v78 in the complete immutable
...
The watcher module and source manifest v78 ship with the full immutable package. Startup hook
...
Worker artifact import and source manifest v78 ship with the complete immutable release. There
...
Worker bundle metadata/submission and source manifest v78 ship as one immutable package.
...
Streamed worker transport and progress modules ship with source manifest v78 as one immutable
...
Remote training and worker compatibility modules ship with source manifest v78 as one immutable
...
v78. The three relocated business source files are included for new task fingerprints; stored model
snapshots remain intact. No route, migration, worker topology or release command changes.

Background Codex transport, task runner and submission ship together with source manifest v78.
...
The three background write modules ship together with source manifest v78, which records their
...
Source manifest v78 includes their actual sources for new task fingerprints and leaves historical
...
Source manifest v78 tracks the moved query/mutation and image-control adapter sources for new fingerprints without rewriting
...
RunPod transfer service/store/API move together in the immutable release. Source manifest v78
...
v78 includes the moved launch and status business files; historical task fingerprints remain intact.
...
CI includes training input/state and immutable preview-fingerprint contracts. Source manifest v78
...
with source manifest v78 in one immutable release. No route/schema, migration, model policy or
...
Ship renderer, its typed ports and source manifest v78 in one immutable package. No API, model,
...
CI includes preview layout contracts. Publish geometry, masks, placement and source manifest v78
...
modules and source manifest v78 in the same immutable release. No dependency, migration, route,
...
with source manifest v78 in the complete immutable release. Routes, storage schema and process
...
source manifest v78 with the whole release. No database migration, endpoint, deployment service,
...
with source manifest v78. This batch changes no request/worker contract, dependency, service,
...
manifest v78 as part of the same immutable release. There is no database migration, worker
...
training modules together with source manifest v78. Training still runs through the existing
...
with source manifest v78 in the same immutable package. No migration, environment setting,
...
source manifest advances to v78 to cover all three moved files. Rollback remains
...
Ship the two stores, pure state policy and source manifest v78 in the whole release.
...
commands and topology remain unchanged. Source manifest v78 now includes both
...
new modules and source manifest v78 with the complete immutable release. This batch
...
warmup modules and manifest v78 together. Worker topology and release commands remain.
...
new modules with manifest v78. It retains the current warmup entry points, in-process
...
resource and detection endpoint checks. Manifest v78 records the new model-input
...
callbacks preserve argument evaluation and TypeError. Source manifest v78 covers
...
five OCR modules with source manifest v78. Local factories never load real models in
...
model parameters, worker topology and release commands. Ship source manifest v78
...
Source manifest v78 records the relocated OCR input orchestrator for new snapshots.
...
Ship the analysis, comparison-cache and HTTP modules with manifest v78 and the root
...
checks. Ship the submission, review, API and port modules plus manifest v78 together;
...
Manifest v78 and normal whole-release restart/rollback apply; no new topology or
...
both media/image modules with source manifest v78, which covers their migrated
...
Preparation CI adds synthetic workflow contracts and source manifest v78 includes
