...
2. **UI consolidation (current):** remove the legacy creation affordance while preserving existing legacy tasks and rollback-compatible routes. The label bench accepts either a live camera capture or an uploaded PNG/JPG/WEBP as the actual image. Its camera input lists available video devices, supports explicit switching, refreshes after device changes and keeps capture disabled while a stream is opening or unavailable. Permission denial or device removal leaves the uploaded-image alternative available. Both reference and actual preview stages provide an enlarged, zoomable inspection view. The standard library is a master/detail workflow: import uses a modal, while every add, select, soft-disable, re-enable and initial-confirm action is available directly inside the expanded order. Every extracted label has a thumbnail plus an enlarged inspection view. Choosing a confirmed library asset is sufficient to supply the reference; no duplicate standard-image upload is required. Standard documents, appended images, optional local reference images and actual images all use the shared keyboard-accessible chooser/drop target, followed by their domain-specific validation. The workspace omits the former persistent scope-warning badge; the authoritative inspection limits remain documented below and enforced by the service gates.
...
The client keeps exactly one reference source: either a library asset or a local image, never both. Reference and actual image slots are independent, so choosing an order asset or uploading a local reference never clears an existing actual image, and replacing the actual image never clears the reference. Any input change still clears stale comparison output and identity; an unchanged retry may reuse its comparison identity, but changed inputs must not. Draft assets are order-editing inputs and cannot be submitted as production comparison standards.
