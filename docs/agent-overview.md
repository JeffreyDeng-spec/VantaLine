...
- `local_inspection_service/model_providers/`: OpenAI-compatible, Gemini, Agnes and Qwen image transports, provider selection/retry orchestration, legacy migration settings, configuration policy, key registry, explicit capabilities, shared error types and payload/error parsing without importing the Web application.
