...
- **Text inspection v2:** an account-scoped standard library and inspection workflow, independent from product/YOLO tasks. New label comparisons enter only through this workspace and accept a browser camera capture or uploaded image; both comparison previews can open an enlarged zoom view. The former persistent scope-warning badge is not rendered, while the service-side verification gates and documented limits remain unchanged. Existing legacy incoming-text tasks remain readable, but their creation affordance is retired. DOCX label candidates are reversible until an immutable version is confirmed; PDF pages are rendered lazily. See `docs/text-inspection-v2.md`.
