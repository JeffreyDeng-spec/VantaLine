...
(async () => {
  ...
  try {
    ...
    await page.route('**/api/**',r=>{
      ...
      if(url.endsWith('/extraction-capabilities'))return r.fulfill({json:{enabled:false}});
    ...
    assert.equal(submitted,1);assert.equal(polls,1);
