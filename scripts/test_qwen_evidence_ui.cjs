...
(async () => {
  ...
  try {
    ...
    await page.locator('.comparison-dialog:not(.comparison-history-dialog) .text-compare-lightbox').waitFor();
    ...
    await page.getByAltText('原分辨率实拍证据').evaluate(img => img.complete ? undefined : new Promise(resolve => img.addEventListener('load',resolve,{once:true})));
    assert.equal(originalRequests,1); assert.equal(previewRequests,1);
    await page.getByRole('button',{name:'返回快速预览',exact:true}).click();
    await page.locator('.comparison-dialog:not(.comparison-history-dialog)').getByText('100% · 结果已保存',{exact:true}).waitFor();
    ...
    assert.equal(await page.locator('.comparison-dialog:not(.comparison-history-dialog)').evaluate(el=>el.scrollWidth<=el.clientWidth+1),true);
    for(const mode of ['malformed','legacy']) {
      evidenceMode=mode; await page.reload();
      await page.locator('.comparison-dialog:not(.comparison-history-dialog)').getByAltText('标准元素位置',{exact:true}).waitFor();
