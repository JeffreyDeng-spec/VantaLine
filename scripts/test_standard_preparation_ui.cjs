...
(async () => {
  ...
  try {
    ...
    progress.job.state='processing';
    progress.items[0].attempt.state='supplementing';
    progress.items[0].attempt.diagnostics={recovery:{regions:[{id:'r1',region_url:'/fixture/region',ocr_url:'/fixture/local-ocr'}]}};
    await page.reload();
    await page.getByRole('button',{name:/标准准备测试/}).click();
    await page.getByText(/第 1 张 · 局部补识别漏检文字/).click();
    assert.equal(await page.getByLabel('识别内容',{exact:true}).isDisabled(),true);
    assert.equal(await page.getByRole('button',{name:'确认修改并启用新版本'}).isDisabled(),true);
    await page.getByText('Raw Output（默认折叠）',{exact:true}).click();
    await page.getByAltText('r1 局部 OCR 元素',{exact:true}).waitFor();
    await page.screenshot({path:path.join(output,'mobile-supplementing.png'),fullPage:true});
    assert.equal(starts,1,'restore supplementation does not call provider');
