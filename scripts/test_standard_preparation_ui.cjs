  ...
  try {
    ...
    await page.route('**/api/**', route=>{
      ...
      if(/\/preparation\/asset\d\/confirm$/.test(url)) {
        ...
        const savedAsset=assets.find(a=>a.id===id);
        savedAsset.comparison_ready=!body.allow_graphics_only;
        savedAsset.comparison_unavailable_reason=body.allow_graphics_only ? '纯图形标准：已保存，当前不支持文字对比' : undefined;
    ...
    // Pure graphics requires explicit acknowledgement, not an arbitrary keep.
    progress.job.state='completed';
    const graphic=progress.items.find(i=>i.id==='asset1');
    graphic.attempt={state:'ready',elements:[],result:{kind:'label_design',coverage_complete:true},diagnostics:{ok:true}};
    graphic.revisions.find(r=>r.id===graphic.draft).elements=[];
    await page.reload(); await page.getByRole('button',{name:/标准准备测试/}).click();
    await page.getByRole('button',{name:'查看大图',exact:true}).first().click(); await dialog(1).waitFor();
    assert.equal(await page.getByRole('button',{name:'保存',exact:true}).isDisabled(),true);
    await page.getByRole('checkbox',{name:/确认主体仅含图形/}).check();
    await page.screenshot({path:path.join(output,'graphics-only-confirmation.png'),fullPage:true});
    await page.getByRole('button',{name:'保存',exact:true}).click();
    await page.getByRole('dialog').waitFor({state:'hidden'});
    await page.getByText('纯图形标准：已保存，当前不支持文字对比',{exact:true}).waitFor();
    assert.equal(saves.at(-1).body.allow_graphics_only,true);
    assert.equal(await page.getByRole('button',{name:'选择第 1 张标签作为对比标准'}).count(),0);
    graphic.attempt.diagnostics={ok:false};
    await page.reload(); await page.getByRole('button',{name:/标准准备测试/}).click();
    await page.getByRole('button',{name:'查看大图',exact:true}).first().click(); await dialog(1).waitFor();
    await page.getByRole('alert').filter({hasText:'不能将识别失败当作纯图形标准'}).waitFor();
    assert.equal(await page.getByRole('checkbox',{name:/确认主体仅含图形/}).count(),0);
    assert.equal(await page.getByRole('button',{name:'保存',exact:true}).isDisabled(),true);
