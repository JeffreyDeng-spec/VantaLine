...
(async () => {
  ...
  await context.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/label-inspection/tasks') return reply({items:[{id:'task-'+user.id,name:'订单-'+user.id,source:'word',standard_count:1,run_count:0,status:'ready',decision:'REVIEW_REQUIRED',updated_at:1}],next_cursor:null});
  ...
  await page.getByRole('heading', { name: '检测任务', exact: true }).waitFor();
