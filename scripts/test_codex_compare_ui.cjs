...
(async()=>{
 ...
 try {
  ...
  task={...task,report_version:'label-v2',status:'running',sequence:20,skill_version:'fixture-skill',progress:{elements:1,total:2,settled:1},counts:{match:0,difference:1,uncertain:0},elements:[{id:'E1',category:'text',name:'额定电压',description:'电压文字',reference:{box:[.08,.35,.6,.2]},actual:{box:[.08,.35,.6,.2]}}],checks:[{id:'C1',element_ids:['E1'],dimension:'text',expected:'220 V',observed:'230 V',status:'difference',explanation:'数值不同',artifact_ids:[],decode_ids:[]},{id:'C2',element_ids:[],dimension:'color',expected:'black and white',status:'pending',observed:'',explanation:'',artifact_ids:[],decode_ids:[]}],issues:[{id:'I1',check_id:'C1',title:'电压错误',explanation:'<script>window.evil=true</script>',reference:{box:[.08,.35,.6,.2]},actual:{box:[.08,.35,.6,.2],polygon:[[.08,.35],[.68,.35],[.68,.55],[.08,.55]]},artifact_ids:[],resolved:false}]};
  await page.reload();await page.getByText('元素与检查清单',{exact:true}).waitFor();
  ...
  assert.equal(await page.evaluate(()=>window.evil),undefined);
  await page.screenshot({path:output+'/label-v2-mobile.png',fullPage:true});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
  await page.setViewportSize({width:1360,height:900});
  await page.screenshot({path:output+'/label-v2-desktop.png',fullPage:true});
