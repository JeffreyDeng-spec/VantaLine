...
(async()=>{
 ...
 try{
  ...
  let task=null,uploads=0,submitted=0,created=0,forbidden=false;
  const oldTask={id:'old-task',report_version:'text-v1',status:'completed',created_at:1789359000,sequence:1,finalized:true,inputs:{standard_name:'历史检查任务',standard_revision_number:1,reference:evidence,actual:evidence},counts:{difference:3,uncertain:0,match:2},summary:{decision:'DIFFERENCES',message:'历史记录保留',checked_scope:'文字',unchecked_scope:''},items:[],artifacts:[],reviews:[]};
  const earlier={...oldTask,id:'earlier-task',inputs:{...oldTask.inputs,standard_name:'更早检查任务'}};
  ...
  await page.route('**/api/**',async r=>{
   ...
   if(url==='/api/text-compare-codex/tasks')return r.fulfill({json:new URL(r.request().url()).searchParams.get('before')?{items:[earlier],next_cursor:null}:{items:[...(task?[task]:[]),oldTask],next_cursor:'older'}});
   if(url.endsWith('/old-task')||url.endsWith('/earlier-task'))return r.fulfill({json:url.endsWith('/old-task')?oldTask:earlier});
   if(url===root){if(method==='POST'){created++;task={report_version:'label-batch-v3',id:'batch-1',status:'draft',created_at:1789360000,sequence:1,summary:null,inputs:{references:{}},references:[],labels:[],counts:{pending:0,difference:0,uncertain:0,match:0}};return r.fulfill({json:task});}return r.fulfill({json:{items:task?[task]:[],next_cursor:null}});}
   if(url.endsWith('/document')||url.endsWith('/order')){task.inputs={standard_id:'std',standard_name:'电源标签订单',references:{R:{id:'R',name:'标准图 1',media:evidence,sources:[{id:'R'}]}}};task.import_state='ready';return r.fulfill({json:task});}
  ...
  assert.equal(await page.locator('.bw-bench').count(),0);
  assert.equal(await page.getByText('旧版单枚报告',{exact:true}).count(),0);
  await page.locator('.bw-task-row').filter({hasText:'历史检查任务'}).click();
  await page.getByText('历史记录保留',{exact:true}).waitFor();
  ...
  await page.getByRole('button',{name:'返回任务列表',exact:true}).click();
  await page.locator('.bw-task-row').filter({hasText:'电源标签订单'}).waitFor();
  await page.evaluate(()=>localStorage.setItem('vantaline-batch:fixture-owner','batch-1'));
  await page.reload();await page.getByRole('heading',{name:'任务列表',exact:true}).waitFor();assert.equal(await page.locator('.bw-bench').count(),0);
  await page.locator('.bw-task-row').filter({hasText:'电源标签订单'}).click();await page.locator('.bw-actual-gallery figure').nth(2).waitFor();assert.equal(created,1);
  ...
  assert.equal(await page.locator('.cc-items').count(),0);
  assert.equal(await page.locator('input[accept="image/*"]').count(),0);
  await page.getByRole('button',{name:'返回任务列表',exact:true}).click();
  await page.locator('.bw-task-row').filter({hasText:'电源标签订单'}).waitFor();
  assert.match(await page.locator('.bw-task-row').first().innerText(),/电源标签订单/);
  await page.screenshot({path:out+'/task-list-desktop.png',fullPage:true});
  await page.locator('.bw-task-row').filter({hasText:'电源标签订单'}).click();await page.locator('.bw-card.difference').waitFor();
  await page.screenshot({path:out+'/batch-desktop.png',fullPage:true});
  await page.locator('.bw-filters button').filter({hasText:'存在差异'}).click();
  ...
  await page.getByRole('button',{name:'返回任务详情',exact:true}).click();await page.locator('.bw-card.difference').waitFor();assert.equal(await page.locator('.bw-card').count(),1);
  ...
  await page.getByRole('button',{name:'返回任务列表',exact:true}).click();await page.locator('.bw-task-row').first().waitFor();
  await page.screenshot({path:out+'/task-list-mobile.png',fullPage:true});
  await page.getByRole('button',{name:'新建任务',exact:true}).click();await page.getByRole('heading',{name:'新建任务',exact:true}).waitFor();
  assert.equal(await page.locator('.bw-actual-gallery figure').count(),0);assert.equal(created,1);
  await page.getByRole('button',{name:'返回任务列表',exact:true}).click();
  ...
  assert.deepEqual(errors,[]);console.log('Batch UI: task list, unified history, pagination, new task, equal panels, parent navigation, upload, draft restore, single submit, markers, account isolation OK');
