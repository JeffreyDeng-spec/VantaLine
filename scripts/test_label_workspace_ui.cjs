...
(async()=>{
 ...
 const task={id:'test-task',name:'测试订单',revision:1,assets:[{id:'a1',name:'标准 1',ordinal:1,enabled:true,media}],runs:[]};
 let submits=0,docs=0,diagnostics=0;
 ...
 await context.route('**/api/**',async route=>{
  ...
  if(p==='/api/label-inspection/tasks'){
   if(req.method()==='POST'){docs++;return reply(task);}
 ...
 await page.getByRole('heading',{name:'测试订单',exact:true}).waitFor();
