...
def register(app: FastAPI, access: HistoryAccess, records: HistoryRecords, evidence: HistoryMedia):
    def owner():
        access.require_permission("inspection", detail="没有文字检验权限")
        return access.owner()[0]

    def owned(record_id, uid):
        record = records.owned("records", record_id, uid)
        if not record:
    ...
    def listing(q: str = Query("", max_length=120), result: str = Query("all"),
        ...
        if repo is not None:
            ...
            standards = {r["id"]: r for r in records.load("standards") if r.get("owner_user_id") == uid}
            rows = [project(r, standards.get(r.get("standard_id"))) for r in records.load("records") if r.get("owner_user_id") == uid]
    ...
    def detail(record_id: str):
        ...
        summary = project(record, records.owned("standards", record.get("standard_id", ""), uid))
        value = records.public(record)
    ...
    def diagnostics(record_id: str):
        return records.public(owned(record_id, owner())).get("diagnostics", {})
    ...
    def media(record_id: str, kind: str):
        ...
        def read(path, sha):
            return evidence.read_verified(path or "", uid, sid, expected_sha256=sha or "")
        ...
        if kind == "reference":
            if record.get("reference_overlay_path"):
                ...
                revision = records.owned("revisions", record.get("standard_revision_id", ""), uid)
                ...
                if prep:
                    ...
                    path = evidence.path(uid, sid, f"preparation_{prep['id']}_clean.png")
                    data = read(str(path), prep["sha256"])
                else:
                    asset = records.owned("assets", snap["id"], uid)
