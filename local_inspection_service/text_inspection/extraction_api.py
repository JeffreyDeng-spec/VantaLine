...
def register(app: FastAPI, access: ExtractionAccess, records: ExtractionRecords,
    ...
    def optional(identifier, uid):
        ...
        if repository is None:
            return records.owned("extractions", identifier, uid)
    ...
    def rows(uid, root=None):
        ...
        return [v for v in records.load("extractions") if v.get("owner_user_id") == uid and (root is None or v.get("root_id") == root)]
    ...
    def save(value, once=False):
        return records.save("extractions", value, insert_only=once)
    ...
    def read(value, kind):
        return media_dependencies.read_verified(value.get(kind+"_path", ""), value["owner_user_id"], value["root_id"], expected_sha256=value.get(kind+"_sha256", ""), max_bytes=100*1024*1024)

    def write(value, kind, data):
        path = media_dependencies.path(value["owner_user_id"],value["root_id"],value["id"]+"-"+media_dependencies.digest(data)[:24]+"-"+kind+".png")
        media_dependencies.write(path,data)
        value[kind+"_path"],value[kind+"_sha256"] = str(path),media_dependencies.digest(data)
    ...
    def run(value, settings, original):
        ...
        try:
            ...
            stage = "provider"
            provider = models.image_provider(settings)
            response = provider.generate_image(prompt,[{"type":"image_url","image_url":{"url":media_dependencies.data_url(data,"image/png")}}],model=settings["model"])
            diagnostic["provider_result"] = models.diagnostic_value({k:response.get(k) for k in ("text","usage_metadata","latency_ms","model")})
    ...
    def run_bbox(value, settings, original):
        ...
        try:
            ...
            stage = "provider"
            body = label_bbox.request_once(data, settings, models.transport)
            # Sanitize parsed fields, not the JSON string, so nested secret keys
            # cannot bypass the existing diagnostic redaction.
            diagnostic["provider_result"] = models.diagnostic_value(label_bbox.evidence(body, settings.get("api_key", "")))
            stage = "bbox_validation"
            result, rect = label_bbox.parse(body)
            diagnostic["parsed_result"] = models.diagnostic_value(label_bbox.evidence(json.dumps(result), settings.get("api_key", "")))
    ...
    def bbox_settings():
        return {**models.detection_settings("document"), "timeout_seconds":label_bbox.TIMEOUT}
    ...
    def expire_drafts(uid):
        ...
        pinned.update(v.get("diagnostics",{}).get("extraction",{}).get("root_id") for v in records.load("records") if v.get("owner_user_id") == uid)
    ...
    def capabilities():
        ...
        settings = models.image_settings()
        bbox = bbox_settings() if bbox_enabled(uid) else {}
        return {"enabled":uid in allowed, "ai_available":uid in allowed and models.external_enabled() and bool(settings.get("configured")), "provider":settings.get("provider"), "model":settings.get("model"), "bbox_enabled":uid in allowed and bbox_enabled(uid), "bbox_available":uid in allowed and bbox_enabled(uid) and models.external_enabled() and bbox.get("provider") == "qwen" and bool(bbox.get("configured"))}

    @app.post("/api/text-inspection/extractions")
    ...
    @app.get("/api/text-inspection/extractions/{identifier}/media/{kind}")
    ...
    @app.post("/api/text-inspection/extractions/{identifier}/revise")
    async def revise(identifier: str, request: Request):
        ...
        if confirmation:
            asset = records.owned("assets",str(body.get("standard_asset_id", "")),uid)
            standard = records.owned("standards",asset["standard_id"],uid) if asset else None
        ...
        path = media_dependencies.path(uid,parent["root_id"],uuid.uuid4().hex+"-crop.png")
