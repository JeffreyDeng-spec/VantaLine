...
from .document_ports import DocumentRecords, DocumentModels, Record
...
class DocumentJobs:
    ...
    def mutate(self, identity, owner, change):
        ...
        with self.records.guard():
            ...
            assets = [a for a in self.records.load('assets') if a.get('standard_id') == identity and a.get('owner_user_id') == owner]
    ...
    def settings(self, owner):
        ...
        if owner not in allowed or not self.models.external_enabled():
        ...
        try:
            ...
            for asset in sorted(assets, key=lambda a: a.get('ordinal', 0)):
                ...
                if digest and digest in cache:
                    ...
                    try:
                        data = self.asset_bytes(target, owner)
                        preview = classifier.prepare_image(data)
                        value, diagnostic = classifier.classify_once(preview, target.get('context', ''), settings, self.models.transport,
                            record_usage=self.models.record_usage)
