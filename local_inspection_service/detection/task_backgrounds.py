...
def ai_detection_task_background_record(task_id: str, *,
        find_task: Callable[[str], Record | None], normalize_background: Callable[[str], str]) -> tuple[str, dict[str, Any]]:
...
def hydrate_auto_optimize_background_from_ai_task(state: dict[str, Any], *,
        background_record: Callable[[str], tuple[str, Record]], normalize_background: Callable[[str], str]) -> bool:
