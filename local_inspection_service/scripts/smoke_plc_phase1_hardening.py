...
async def run_in_test_thread(function: Any, *args: Any, **kwargs: Any) -> Any:
    """Sandbox-safe replacement for asyncio.to_thread used by concurrency probes."""
    result: dict[str, Any] = {}
    ...
    def invoke() -> None:
        ...
        except BaseException as exc:  # pragma: no cover - re-raised on the test loop
            failure.append(exc)
...
async def wait_event_async(event: threading.Event, timeout: float = 2.0) -> bool:
...
async def wait_predicate_async(predicate: Callable[[], bool], timeout: float = 2.0) -> bool:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if predicate():
            return True
...
def enabled_config(**overrides: Any) -> dict[str, Any]:
    ...
    legacy_write = migrated.pop("write_y04", None)
    legacy_d = migrated.pop("d206_address", None)
    legacy_y = migrated.pop("y04_address", None)
    if legacy_d is not None:
        migrated["result_register"] = legacy_protocol_address_to_device(legacy_d, "D")
    if legacy_write is not None:
        migrated["output_control_point"] = (
            legacy_protocol_address_to_device(legacy_y or "0108", "Y") if legacy_write else ""
        )
    elif legacy_y is not None:
        migrated["output_control_point"] = legacy_protocol_address_to_device(legacy_y, "Y")
    return normalize_config({**DEFAULT_PLC_CONFIG, "enabled": True, "serial_port": "COM3", "output_control_point": "", **migrated})
...
def test_protected_namespace_rejects_stale_generic_saves_json_and_pg() -> None:
    ...
    server.plc_pg_coordination_available = lambda: False
    try:
        save_enabled(retries=0)
...
def test_restart_hydration_cas_and_safe_queue_adoption() -> None:
    ...
    server._plc_dispatch_runtime.clear()
    adoption_factory = ScriptedFactory([lambda writes: ScriptedTransport(writes, response=ACK)])
    server._plc_transport_factory = adoption_factory
    try:
        ...
        server._plc_transport_factory = None
    assert recovered["state_version"] > queued_nonterminal["state_version"], recovered
    assert recovered["status"] == "acknowledged"
    assert recovered["attempted"] is True
    assert adoption_factory.writes == [build_d206_frame("119C", False)]
    assert opens == 0
    ...
    server._plc_dispatch_runtime.clear()
    opens_before_uncertain = opens
    server._plc_transport_factory = forbidden
    try:
        uncertain = server.dispatch_plc_for_detection(
            {"request_id": "restart-stale-cas", "passed": True},
            source="image",
            fingerprint="restart-stale-cas",
        )["plc_sync"]
        ...
        server._plc_transport_factory = None
    assert uncertain["error_code"] == "restart_recovery_required"
    assert uncertain["attempted"] is False
    assert opens == opens_before_uncertain
...
def test_pg_non_owner_queue_and_fenced_adoption() -> None:
    backend = SharedPgBackend()
    repository = FakeIndependentPgRepository(backend)
    original_repo = server.runtime_postgres_repository_or_none
    original_activation = server.plc_activation_errors
    original_heartbeat = server.plc_start_owner_heartbeat
    original_owner_id = server._plc_process_owner_id
    writes: list[bytes] = []
    server.runtime_postgres_repository_or_none = lambda: repository
    server.plc_activation_errors = lambda _settings: []
    server.plc_start_owner_heartbeat = lambda _epoch: None
    server._plc_process_owner_id = "worker-b"
    server._plc_transport_factory = lambda _config: ScriptedTransport(writes, response=ACK)
    try:
        save_enabled(generation=61, retries=0)
        now = time.time()
        backend.values[server.PLC_RUNTIME_COORDINATION_KEY] = {
            "io_owner": {
                "owner_id": "worker-a",
                "epoch": 4,
                "heartbeat_at": now,
                "expires_at": now + 30,
                "quarantine_until": now + 90,
            }
        }
        result = {"request_id": "pg-owner-handoff", "passed": True}
        queued = server._run_queued_plc_dispatch(
            dict(result), source="image", fingerprint="pg-owner-handoff"
        )["plc_sync"]
        assert queued["status"] == "queued"
        assert queued["error_code"] == "plc_io_owner_pending"
        assert writes == []
        persisted = audit_for(queued["dispatch_id"])[0]
        assert server.plc_dispatch_is_pristine_queue(persisted)

        owner = backend.values[server.PLC_RUNTIME_COORDINATION_KEY]["io_owner"]
        owner["expires_at"] = now - 1
        owner["quarantine_until"] = now - 1
        adopted = server.plc_reconcile_pending_dispatches_once()
        assert adopted and adopted["status"] == "acknowledged"
        assert writes == [build_d206_frame("119C", True)]
        assert server.plc_reconcile_pending_dispatches_once() is None
        assert writes == [build_d206_frame("119C", True)]
    finally:
        server._plc_transport_factory = None
        server._plc_process_owner_id = original_owner_id
        server.plc_start_owner_heartbeat = original_heartbeat
        server.plc_activation_errors = original_activation
        server.runtime_postgres_repository_or_none = original_repo
...
def test_strict_persisted_config_pollution_and_recovery(admin: Any) -> None:
    ...
    server._plc_transport_factory = lambda _config: None
...
def update_plc_direct(payload: dict[str, Any]) -> dict[str, Any]:
    """Exercise config mutation without the smoke client's single-request serialization."""
    return server.update_plc_config(server.PlcConfigRequest(**payload))
...
async def test_disable_barriers_and_event_loop(admin: Any) -> None:
    ...
    assert await wait_event_async(read_started)
    disabled = update_plc_direct({"enabled": False})
    ...
    status = server.plc_config_response()
    disabled = update_plc_direct({"enabled": False})
    elapsed = time.monotonic() - started
    assert status["in_flight_attempts"]
    assert disabled["config"]["enabled"] is False
...
async def test_generation_epoch_cancels_old_config(admin: Any) -> None:
    save_enabled(retries=1, result_register="D206")
    read_started = threading.Event()
    read_release = threading.Event()
    retry = ScriptedFactory(
        [lambda writes: ScriptedTransport(writes, response=NAK, read_started=read_started, read_release=read_release)]
    ...
    assert await wait_event_async(read_started)
    generation_before = int(server.load_config().get(server.PLC_CONTROL_GENERATION_KEY) or 0)
    changed = update_plc_direct({"result_register": "D207"})
    assert changed["control_generation"] == generation_before + 1
    read_release.set()
    result = await task
    ...
    assert await wait_event_async(read_started)
    generation_before = int(server.load_config().get(server.PLC_CONTROL_GENERATION_KEY) or 0)
    changed = update_plc_direct(
        {"checksum_mode": "include_etx_documented_comment", "output_control_point": ""}
    )
    assert changed["control_generation"] == generation_before + 1
    read_release.set()
    result = await task
    ...
    generation_before = int(server.load_config().get(server.PLC_CONTROL_GENERATION_KEY) or 0)
    same_address = server.load_config()["plc"]["result_register"]
    equal = update_plc_direct({"result_register": same_address})
    assert equal["control_generation"] == generation_before
    server._plc_transport_factory = None
...
async def test_queue_timeout_and_queue_disable(admin: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            assert await wait_predicate_async(wait_until_queued)
            disabled = update_plc_direct({"enabled": False})
            assert disabled["config"]["enabled"] is False
...
async def test_attempt_declaration_linearization(admin: Any) -> None:
    ...
    save_enabled(generation=90, retries=0, result_register="D206")
    ...
    try:
        ...
        assert await wait_event_async(before_start)
        changed = update_plc_direct({"result_register": "D207"})
        assert changed["control_generation"] == 91
        start_release.set()
    ...
    save_enabled(generation=100, retries=0, result_register="D206", output_control_point="Y04")
    ...
    try:
        ...
        assert await wait_event_async(after_start)
        changed = update_plc_direct(
            {"result_register": "D207", "output_control_point": ""}
        )
        payload = changed
...
async def test_total_deadline_before_and_after_attempt() -> None:
    ...
    try:
        ...
        assert await wait_event_async(before_gate)
        ...
        gate_release.set()
        assert await wait_event_async(worker_finished)
...
def main() -> None:
    ...
    test_restart_hydration_cas_and_safe_queue_adoption()
    test_pg_non_owner_queue_and_fenced_adoption()
