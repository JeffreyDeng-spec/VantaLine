...
def test_settings_api_persistence_roundtrip_permissions_and_validation() -> None:
    server._plc_transport_factory = SequenceFactory([ACK])
    ...
    expected_config = normalize_config(saved_payload)
    assert saved.json()["config"] == expected_config
    reloaded = admin.get("/api/plc/config")
    assert_status(reloaded, 200, "reload PLC config")
    assert reloaded.json()["config"] == expected_config
    assert server.load_config()["plc"] == expected_config
