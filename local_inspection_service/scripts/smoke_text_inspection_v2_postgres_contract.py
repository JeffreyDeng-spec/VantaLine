...
def main() -> None:
    now = 1_800_000_000
    samples = {
        "document_imports": {"id": "doc_1", "root_id": "doc_1", "owner_user_id": "user_1", "created_at": now + .25, "kind": "root"},
    ...
    for kind, value in samples.items():
        ...
        if kind == "document_imports":
            assert isinstance(row["raw_json"], dict) and row["root_id"] == value["root_id"]
            assert row["created_at"] == now and row["raw_json"]["created_at"] == now + .25
