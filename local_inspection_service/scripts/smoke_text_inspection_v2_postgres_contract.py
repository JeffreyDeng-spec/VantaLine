...
def main() -> None:
    now = 1_800_000_000
    samples = {
        "ocr_evidence": {"id": "ocr_1", "owner_user_id": "user_1", "created_at": now, "status": "attempting"},
    ...
    for kind, value in samples.items():
        ...
        if kind in {"extractions", "ocr_evidence"}:
            assert isinstance(row["raw_json"], dict), "owned evidence queries require a JSONB object"
