...
from local_inspection_service.storage.schema import TEXT_INSPECTION_SCHEMA_VERSION  # noqa: E402
...
def main() -> None:
    ...
    migration = (ROOT / "local_inspection_service" / "storage" / "migrations" / f"{TEXT_INSPECTION_SCHEMA_VERSION}.sql").read_text(encoding="utf-8")
    assert "UNIQUE (owner_user_id, comparison_id)" in migration
    assert "UNIQUE (owner_user_id, session_id, capture_id)" in migration
    assert "DROP TABLE" not in migration.upper() and "ALTER TABLE" not in migration.upper()
    assert "pg_advisory_xact_lock" in Path(server.__file__).with_name("storage").joinpath("postgres_runtime_repository.py").read_text(encoding="utf-8")
