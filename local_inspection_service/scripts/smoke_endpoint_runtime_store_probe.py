...
def assert_postgres_count_probe_failure_is_redacted() -> None:
    class FailingCountRepository:
        kind = "postgres"

        def count_rows(self, _table_names: tuple[str, ...]) -> dict[str, int]:
            raise RuntimeError("postgresql://user:probe-secret@example.local/vantaline")

    original_selection = server.runtime_repository_selection
    server.runtime_repository_selection = lambda: SimpleNamespace(
        store=server.POSTGRES_STORE,
        repository=FailingCountRepository(),
    )
    try:
        try:
            server.runtime_store_probe_payload()
        except HTTPException as exc:
            detail = exc.detail if isinstance(exc.detail, dict) else {}
            if exc.status_code != 503 or detail.get("code") != "runtime_store_probe_error":
                raise AssertionError(f"expected count probe failure, got {exc.status_code}: {exc.detail}") from exc
            if detail.get("message") != "RuntimeError":
                raise AssertionError(f"count probe must expose only the exception type: {detail}") from exc
            if "probe-secret" in str(detail):
                raise AssertionError(f"count probe leaked secret exception text: {detail}") from exc
            if detail.get("json_fallback_used") is not False:
                raise AssertionError(f"count probe failure must remain fail-closed: {detail}") from exc
        else:
            raise AssertionError("failing PostgreSQL count probe must return HTTP 503")
    finally:
        server.runtime_repository_selection = original_selection
...
def main() -> None:
    ...
    assert_postgres_count_probe_failure_is_redacted()
