...
WORKER_DOCKERFILE = SERVICE_DIR / "workers" / "vantaline_yolo_train_worker" / "Dockerfile"
WORKER_REQUIREMENTS = SERVICE_DIR / "workers" / "vantaline_yolo_train_worker" / "requirements.txt"
...
def main() -> None:
    ...
    assert_true("FROM pytorch/pytorch:2.4.1-cuda12.1-cudnn9-runtime@sha256:" in dockerfile, "worker base image must use an immutable digest")
    assert_true("pip install --upgrade pip" not in dockerfile, "worker build must not float pip during image creation")
    for expected_requirement in ("runpod==1.11.0", "ultralytics==8.4.66", "requests==2.34.2"):
        assert_true(expected_requirement in requirements.splitlines(), f"worker dependency must be pinned: {expected_requirement}")
