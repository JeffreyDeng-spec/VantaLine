export function RereadEvidence({ value, recordId, onZoom }: { value: unknown; recordId: string; onZoom?: (src: string, alt: string) => void }) {
  ...
  return <section aria-label="局部复读输入证据">
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
      {traces.map(t => <figure key={t.id} style={{ margin: 0, maxWidth: 280 }}>
        <a href={`/api/text-inspection/prepared-comparisons/${recordId}/media/${t.id}`} target="_blank" rel="noreferrer" onClick={event => { if (onZoom) { event.preventDefault(); onZoom(event.currentTarget.href, "局部复读输入证据"); } }}>
