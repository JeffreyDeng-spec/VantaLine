...
import type { AgentConfigResponse, AiConfigResponse, AiKeySummary, ApiCostDailyPoint, ApiCostLedgerResponse, PlcConfig } from "../../api/types";
...
function readPlcPayload(form: HTMLFormElement): Partial<PlcConfig> {
  const data = new FormData(form);
  return {
    enabled: data.get("enabled") === "on",
    protocol: "fx_programming_port_ascii",
    checksum_mode: String(data.get("checksum_mode") || "exclude_etx_legacy_vb") as PlcConfig["checksum_mode"],
    serial_port: String(data.get("serial_port") || "").trim(),
    baudrate: Number(data.get("baudrate") || 9600),
    parity: String(data.get("parity") || "E") as PlcConfig["parity"],
    data_bits: Number(data.get("data_bits") || 7) as PlcConfig["data_bits"],
    stop_bits: Number(data.get("stop_bits") || 1) as PlcConfig["stop_bits"],
    d206_address: String(data.get("d206_address") || "119C").trim().toUpperCase(),
    y04_address: String(data.get("y04_address") || "0108").trim().toUpperCase(),
    write_y04: data.get("write_y04") === "on",
    timeout: Number(data.get("timeout") || 1),
    retries: Number(data.get("retries") || 0)
  };
}
...
export function RulesPage() {
  ...
  const plcQuery = useQuery({
    queryKey: queryKeys.plcConfig,
    queryFn: getPlcConfig,
  ...
  const plcMutation = useMutation({
    mutationFn: savePlcConfig,
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.plcConfig });
      notify({ title: "PLC 设置已保存", description: "新设置会从下一次图片或视频最终结果开始生效。", tone: "success" });
    },
  ...
  function handlePlcSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    plcMutation.mutate(readPlcPayload(event.currentTarget));
  }
        ...
        <section className="panel page-panel">
          <div className="section-title">
            ...
            <span className={`pill ${plcResponse?.effective_enabled ? toneForStatus(latestPlcDispatch?.status) : "neutral"}`}>
              {plcResponse?.effective_enabled ? plcStatusLabel(latestPlcDispatch?.status) : "有效状态：关闭"}
            </span>
          </div>
          {plcQuery.isLoading ? (
            ...
            <>
              <div className="settings-subhead">
                <div>
                  <h4>Mitsubishi FX programming-port ASCII</h4>
                  <p>仅在图片或视频最终结论生成后写入；关闭时不会打开串口，PLC 失败也不会改变检测结论。</p>
                </div>
                <span className={`pill ${plcResponse?.effective_enabled ? "ok" : "neutral"}`}>{plcResponse?.effective_enabled ? "已启用" : "已关闭 / 被阻断"}</span>
              </div>
              {plcResponse?.validation_error ? <p className="hint-line danger-text">配置校验失败：{plcResponse.validation_error}</p> : null}
              {(plcResponse?.validation_errors || []).map((item) => (
                <p className="hint-line danger-text" key={item.code}>{item.code}：{item.message}</p>
              ))}
              <form
                className="settings-form"
                ref={plcFormRef}
                key={`${plc.enabled}-${plc.serial_port}-${plc.d206_address}-${plc.y04_address}-${plc.write_y04}-${plc.checksum_mode}`}
              ...
              >
                <div className="form-grid settings-option-grid">
                  <label className="toggle-row">
                    ...
                    <span>启用最终检测结果同步</span>
                  </label>
                  <label className="toggle-row">
                    <input name="write_y04" type="checkbox" defaultChecked={plc.write_y04} />
                    <span>同时写入 Y04 输出</span>
                  </label>
                </div>
                <div className="form-grid">
                  <label className="field">
                    Protocol
                    <select name="protocol" defaultValue={plc.protocol} disabled>
                      <option value="fx_programming_port_ascii">FX programming-port ASCII</option>
                    </select>
                  </label>
                  <label className="field">
                    Serial port
                    <input name="serial_port" defaultValue={plc.serial_port} placeholder="COM3 或 /dev/ttyUSB0" autoComplete="off" />
                  </label>
                  <label className="field">
                    Checksum mode
                    <select name="checksum_mode" defaultValue={plc.checksum_mode}>
                      <option value="exclude_etx_legacy_vb">Legacy VB executable：不含 ETX（默认）</option>
                      <option value="include_etx_documented_comment">VB 注释描述：包含 ETX</option>
                    </select>
                  </label>
                  <label className="field">
                    Baudrate
                    <select name="baudrate" defaultValue={plc.baudrate}>
                      {[1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200].map((value) => (
                  ...
                  <label className="field">
                    Parity
                    <select name="parity" defaultValue={plc.parity}>
                  ...
                  <label className="field">
                    Data bits
                    <select name="data_bits" defaultValue={plc.data_bits}>
                  ...
                  <label className="field">
                    Stop bits
                    <select name="stop_bits" defaultValue={plc.stop_bits}>
                  ...
                  <label className="field">
                    D206 address
                    <input name="d206_address" pattern="[0-9A-Fa-f]{4}" maxLength={4} defaultValue={plc.d206_address} />
                  </label>
                  <label className="field">
                    Y04 address
                    <input name="y04_address" pattern="[0-9A-Fa-f]{4}" maxLength={4} defaultValue={plc.y04_address} />
                  </label>
                  <label className="field">
                    Timeout / 秒
                    <input name="timeout" type="number" min="0.1" max="10" step="0.1" defaultValue={plc.timeout} />
                  </label>
                  <label className="field">
                    Retries
                    <input name="retries" type="number" min="0" max="5" step="1" defaultValue={plc.retries} />
                  </label>
                </div>
                <p className="hint-line danger-text">校验存在原始证据矛盾：VB 可执行循环排除 ETX，但其注释写作包含 ETX；默认遵循可执行代码。包含 ETX 模式仅对应注释，不是 VB 实证。任何模式上线前都必须经过现场真实 PLC ACK gate。</p>
                <p className="hint-line">PASS 写 D206=1（可选 Y04=ON），FAIL 写 D206=0（可选 Y04=OFF）。本页不提供真实 PLC 扫描或测试写入。排队上限 {plcResponse?.queue_wait_seconds}s，请求等待总上限 {plcResponse?.worker_total_timeout_seconds}s。</p>
                <p className="hint-line">关闭 generation：{plcResponse?.control_generation ?? 0}；进行中的底层 I/O：{plcResponse?.in_flight_attempts?.length || 0}。{plcResponse?.disable_notice}</p>
                <div className="button-row">
              ...
              <section className="cost-ledger-section">
                <div className="section-title compact">
                  <h4>最近同步状态</h4>
                  <span className="muted-text">当前显示最近 {plcResponse?.recent_dispatches?.length || 0} 条（幂等记录不随展示窗口淘汰）</span>
                </div>
                <div className="cost-call-table">
                  <div className="cost-call-row head">
                    <span>时间</span>
                  ...
                  {!plcResponse?.recent_dispatches?.length ? <div className="cost-empty-state">还没有已启用的同步记录。</div> : null}
                </div>
              </section>
            </>
          ) : (
