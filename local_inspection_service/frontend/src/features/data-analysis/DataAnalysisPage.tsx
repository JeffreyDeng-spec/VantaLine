...
import { BarChart3, Eye, FileCheck2, Image as ImageIcon, RefreshCw, X } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import {
  getDataAnalysisRecord,
  getDataAnalysisRecords,
  getIncomingTextInspections,
  queryKeys
} from "../../api/queries";
import type {
...
type DataAnalysisView = "records" | "processing" | "incoming";
...
export function DataAnalysisPage() {
  ...
  const [view, setView] = useState<DataAnalysisView>(() => hasPermission(auth.user, "ai_detection") ? "records" : "incoming");
  ...
  const recordsQuery = useQuery({
    ...
    enabled: hasPermission(auth.user, "ai_detection")
  });
  const incomingQuery = useQuery({
    queryKey: queryKeys.incomingTextInspections(auth.dataUserId),
    queryFn: () => getIncomingTextInspections(auth),
    enabled: hasPermission(auth.user, "inspection")
  });
  ...
  return (
    <section className="view active data-analysis-view">
      ...
      <div className="tabbar data-analysis-tabs" role="tablist" aria-label="数据分析视图">
        {hasPermission(auth.user, "ai_detection") ? <button className={`mode-tab ${view === "records" ? "active" : ""}`} type="button" onClick={() => setView("records")}>
          检测记录
        </button> : null}
        {hasPermission(auth.user, "ai_detection") ? <button className={`mode-tab ${view === "processing" ? "active" : ""}`} type="button" onClick={() => setView("processing")}>
          图片处理
        </button> : null}
        <button className={`mode-tab ${view === "incoming" ? "active" : ""}`} type="button" onClick={() => setView("incoming")}>
          包材文字检验
        </button>
      </div>

      {view !== "incoming" ? <section className="panel data-analysis-toolbar">
      ...
      <section className="panel data-analysis-panel">
        ...
        {view === "incoming" ? (
          <div className="incoming-text-analysis">
            <div className="incoming-text-analysis-summary">
              {["PASS", "FAIL", "REVIEW_REQUIRED", "RELEASED", "REJECTED"].map((decision) => (
                <div key={decision}><span>{decision === "PASS" ? "自动通过" : decision === "FAIL" ? "自动不通过" : decision === "REVIEW_REQUIRED" ? "待复核" : decision === "RELEASED" ? "人工放行" : "人工退货"}</span><strong>{Number(incomingQuery.data?.summary?.[decision] || 0)}</strong></div>
              ))}
            </div>
            {incomingQuery.isLoading ? <LoadingState label="正在读取包材检验记录" /> : null}
            {incomingQuery.isError ? <ErrorState error={incomingQuery.error} action={<button onClick={() => incomingQuery.refetch()}>重试</button>} /> : null}
            {!incomingQuery.isLoading && !incomingQuery.data?.items.length ? <div className="empty-panel"><FileCheck2 size={22} /><strong>暂无包材文字检验记录</strong><span>完成一次包材文字检验后会自动出现在这里。</span></div> : null}
            {incomingQuery.data?.items.length ? <div className="table-wrap"><table><thead><tr><th>时间</th><th>物料</th><th>标准版本</th><th>自动结论</th><th>最终结论</th><th>差异字段</th></tr></thead><tbody>{incomingQuery.data.items.map((item) => <tr key={item.id}><td>{formatRecordTime(item.created_at)}</td><td><strong>{item.material_name || item.material_code}</strong><br /><small>{item.material_code}</small></td><td>{item.reference_version_label}</td><td><span className={`pill ${item.auto_decision === "PASS" ? "ok" : item.auto_decision === "FAIL" ? "fail" : "warn"}`}>{item.auto_decision === "PASS" ? "通过" : item.auto_decision === "FAIL" ? "不通过" : "需复核"}</span></td><td>{item.final_decision || "-"}</td><td>{(item.fields || []).filter((field) => field.outcome !== "PASS").map((field) => field.name).join("、") || "-"}</td></tr>)}</tbody></table></div> : null}
          </div>
        ) : null}
      </section>
